...
export default {
  ...
  methods: {
    ...
    async fetchDemande() {
      ...
      try {
        const token = localStorage.getItem("token");
        const res = await axios.get(`http://localhost:4001/api/demandes/${this.$route.params.id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
    ...
    async updateDemande() {
      ...
      try {
        ...
        await axios.put(`http://localhost:4001/api/demandes/${this.demande.id}`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        if (statutToTraitement && parseInt(this.selectedStatutId, 10) === statutToTraitement.id) {
          try {
            ...
            const genRes = await axios.put(
              `http://localhost:4001/api/demandes/${this.demande.id}/generate-document`,
              {},
              {
                headers: { Authorization: `Bearer ${token}` }
              }
